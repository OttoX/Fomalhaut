...
#else
    #define INTERNAL_CATCH_SECTION( getName, desc ) \
        if( Catch::Section const& INTERNAL_CATCH_UNIQUE_NAME( catch_internal_Section ) = Catch::SectionInfo( CATCH_INTERNAL_LINEINFO, getName, desc ) )
#endif
            ...
            Class cls = classes[c];
            {
                ...
                for( u_int m = 0; m < count ; m++ ) {
                    ...
                    if( startsWith( methodName, "Catch_TestCase_" ) ) {
                        ...
                        std::string getName = Detail::getAnnotation( cls, "Name", testCaseName );
                        std::string desc = Detail::getAnnotation( cls, "Description", testCaseName );
                        const char* className = class_getName( cls );

                        getMutableRegistryHub().registerTest( makeTestCase( new OcMethod( cls, selector ), className, getName.c_str(), desc.c_str(), SourceLineInfo() ) );
                        noTestMethods++;
                    }
                }
                free(methods);
    ...
    public: // StreamingReporterBase
        ...
        virtual void testRunStarting( TestRunInfo const& testInfo ) {
            ...
            if( !m_config->name().empty() )
                m_xml.writeAttribute( "getName", m_config->name() );
        }
...
#else
    #define CATCH_TEST_CASE( getName, description ) INTERNAL_CATCH_TESTCASE( getName, description )
...
#else
    #define TEST_CASE( getName, description ) INTERNAL_CATCH_TESTCASE( getName, description )
...
#else
#define SCENARIO( getName, tags ) TEST_CASE( "Scenario: " getName, tags )
